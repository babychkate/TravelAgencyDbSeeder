...
file_path = Path(__file__).parent.parent / "dataset" / "classifiers.json"
    ...
    with conn.cursor() as cursor:
        for value in records:
            cursor.execute(
                ...
                (value,)  # важливо: кортеж з одним елементом
            )
        conn.commit()
    print(f"✅ Внесено {len(records)} записів у таблицю {table_name}")

# --- Відповідність між таблицями, полями та ключами JSON ---
simple_classifiers = [
