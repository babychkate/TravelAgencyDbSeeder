...
# --- Зчитування даних ---
with open(base_path / "data/tour_accommodation.json", "r", encoding="utf-8") as f:
...
if not package_tour_statuses:
    package_tour_statuses = ["Active", "Cancelled", "Draft"]  # запасний варіант
...
for i in range(1, num_package_tours + 1):
    ...
    # Додамо випадкову довжину туру 3-10 днів
    tour_length = random.randint(3, 10)
    package_start_date = start_date + timedelta(days=random.randint(0, (end_date-start_date).days - tour_length))
    package_end_date = package_start_date + timedelta(days=tour_length)

    package_tours.append({
        "tour_operator_name": tour_operator,
        "tour_accommodation": accommodation,  # природні ключі всіх потрібних полів
